...
import { syncV5CourseToLms, syncV5EnrollmentToLms } from "./v5-sync-helpers.js";

async function courseDeliveryMode(slug) {
  const courseSlug = String(slug || '').trim();
  if (!courseSlug) return '';
  try {
    const { data, error } = await supabase.from('courses').select('delivery_mode').eq('slug', courseSlug).maybeSingle();
    if (error) throw error;
    return String(data?.delivery_mode || '').trim().toLowerCase();
  } catch (error) {
    console.error('[sync] cannot resolve course delivery mode:', error);
    return '';
  }
}

export async function syncCourseToExternalSystems(courseData) {
  const resolvedMode = String(courseData.deliveryMode || courseData.delivery_mode || '').trim().toLowerCase() || await courseDeliveryMode(courseData.slug);
...
export async function syncEnrollmentToExternalSystems(orderData, actionType) {
  const explicitMode = String(orderData.delivery_mode || orderData.deliveryMode || '').trim().toLowerCase();
  const courseSlug = orderData.course_slug || orderData.course;
  const resolvedMode = explicitMode || await courseDeliveryMode(courseSlug);
